...
#ifdef WIN32
	static std::string lua_Dir = szUserDataFolder + "scripts\\lua\\";
	static std::string dzv_Dir = szUserDataFolder + "scripts\\dzVents\\runtime\\";
#else
	static std::string lua_Dir = szUserDataFolder + "scripts/lua/";
	static std::string dzv_Dir = szUserDataFolder + "scripts/dzVents/runtime/";
#endif
...
#else
	dzv_Dir = szUserDataFolder + "scripts/dzVents/generated_scripts/";
	_log.Log(LOG_STATUS,"dzv_Dir Path: %s", dzv_Dir.c_str());
...
void CEventSystem::GetCurrentMeasurementStates()
{
	...
	for (it_type iterator = m_devicestates.begin(); iterator != m_devicestates.end(); ++iterator)
	{
		...
		case pTypeTEMP_HUM_BARO_VOLT:
			...
			isBaro = true;
			isDew = true;
			utilityval = static_cast<float>(atof(splitresults[2].c_str()));
			isUtility = true;
			break;
		case pTypeTEMP_BARO:
			if (splitresults.size() > 1)
...
void CEventSystem::UpdateDevice(const std::string &DevParams)
{
	...
	if (result.size()>0)
	{
		...
		/*if(devType==pTypeLimitlessLights)
		{
			m_mainworker.UpdateDevice(atoi(hid.c_str()), did.c_str(), atoi(dunit.c_str()), devType, atoi(dsubtype.c_str()), atoi(nvalue.c_str()), svalue, 12, 100, true);
		}*/
...
bool CEventSystem::ScheduleEvent(int deviceID, std::string Action, bool isScene, const std::string &eventName, int sceneType)
{
	...
	if ( oParseResults.sCommand.substr( 0, 12 ) == "Set LevelHue" ) {
		std::string param=oParseResults.sCommand.substr( 12 );
		std::vector<std::string> paramrarray;
		StringSplit(param, ";", paramrarray);
		if(paramrarray.size()>=1)
			level = calculateDimLevel( deviceID, atoi(paramrarray[0].c_str() ) );
		if(paramrarray.size()>=2)
			hue = atoi(paramrarray[1].c_str());
		oParseResults.sCommand = oParseResults.sCommand.substr(0, 9);
	} else if ( oParseResults.sCommand.substr( 0, 9 ) == "Set Level" ) {
	...
	for ( int iIndex = 0; iIndex < abs(oParseResults.iRepeat); iIndex++ ) {
		...
		if ( isScene ) {
			...
			tItem = _tTaskItem::SwitchLightEvent( fDelayTime, deviceID, oParseResults.sCommand, level, hue, eventName );
		}
		...
		if (
			...
			if ( isScene ) {
				...
				tDelayedtItem = _tTaskItem::SwitchLightEvent( fDelayTime, deviceID, previousState, previousLevel, hue, eventName );
			}
