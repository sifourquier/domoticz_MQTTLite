...
namespace http {
	namespace server {
		...
		bool CWebServer::StartServer(server_settings & settings, const std::string & serverpath, const bool bIgnoreUsernamePassword)
		{
			...
			RegisterRType("createMQTTsensor", boost::bind(&CWebServer::RType_CreateMQTTsensor, this, _1, _2, _3));
		...
		void CWebServer::Cmd_AddHardware(WebEmSession & session, const request& req, Json::Value &root)
		{
			...
			if (IsSerialDevice(htype))
			{
				...
				(htype == HTYPE_NefitEastLAN) || (htype == HTYPE_DenkoviSmartdenLan) || (htype == HTYPE_Ec3kMeterTCP) || (htype == HTYPE_MultiFun) || (htype == HTYPE_ZIBLUETCP) || (htype == HTYPE_OnkyoAVTCP) || htype == HTYPE_MQTT_LITE
				) {
				//Lan
				if (address == "" || port == 0)
					return;
				...
				if (htype == HTYPE_MQTT || htype == HTYPE_MQTT_LITE) {
					std::string modeqStr = request::findValue(&req, "mode1");
					if (!modeqStr.empty()) {
						mode1 = atoi(modeqStr.c_str());
					}
		...
		void CWebServer::Cmd_UpdateHardware(WebEmSession & session, const request& req, Json::Value &root)
		{
			...
			if (IsSerialDevice(htype))
			{
				...
				||(htype == HTYPE_NefitEastLAN) || (htype == HTYPE_DenkoviSmartdenLan || htype == HTYPE_MQTT_LITE)
				) {
				//Lan
				if (address == "")
					return;
...
-					(dType == pTypeTEMP_HUM_BARO_VOLT)
			...
			else if (cparam == "setcolbrightnessvalue")
			{
				...
				sleep_milliseconds(100);
				m_mainworker.SwitchLight(ID, "Set Brightness", (unsigned char)atoi(brightness.c_str()), atoi(hue.c_str()), false, 0);
				root["status"] = "OK";
			...
			const std::string &hardwareid)
		{
			...
			if (result.size() > 0)
			{
				...
				for (itt = result.begin(); itt != result.end(); ++itt)
				{
						...
						)
					{
						...
						if (switchtype == STYPE_Dimmer)
						{
							...
							if(strarray.size()>1)
							{
								...
								{
									std::string nameValue("LevelInt");
									nameValue+=std::to_string(l+1);
									root["result"][ii][nameValue] = atoi(strarray[l].c_str());
								}
							}
							else
							{
					...
					else if (dType == pTypeTEMP_HUM_BARO_VOLT)
					{
						std::vector<std::string> strarray;
						StringSplit(sValue, ";", strarray);
						if (strarray.size() == 6)
						{
							...
							root["result"][ii]["Temp"] = (int)(temp*10)/10.d;
							...
							root["result"][ii]["TypeImg"] = "Voltage";
							root["result"][ii]["Voltage"] = atof(strarray[5].c_str());
							root["result"][ii]["Barometer"] = atoi(strarray[3].c_str());
							root["result"][ii]["ForecastStr"] = RFX_Forecast_Desc(atoi(strarray[4].c_str()));

							sprintf(szTmp, "%.1f", ConvertTemperature(CalculateDewPoint(tempCelcius, humidity), tempsign));
							root["result"][ii]["DewPoint"] = szTmp;

							sprintf(szData, "%.1f%c %d%% %dhPa %.2fV",
								temp,
								tempsign,
								atoi(strarray[1].c_str()),
								atoi(strarray[3].c_str()),
								atof(strarray[5].c_str())
								);

							//sprintf(szData, "%.3f V", atof(strarray[5].c_str()));
							root["result"][ii]["Data"] = szData;
							sprintf(szData, "%.2fV",
								atof(strarray[5].c_str())
								);
		...
		void CWebServer::RType_HandleGraph(WebEmSession & session, const request& req, Json::Value &root)
		{
				...
				if (sensor == "temp") {
					...
					if (result.size() > 0)
					{
						...
						for (itt = result.begin(); itt != result.end(); ++itt)
						{
							...
							if ((dType == pTypeHUM) || (dType == pTypeTEMP_HUM) || (dType == pTypeTEMP_HUM_BARO) || (dType == pTypeTEMP_HUM_BARO_VOLT))
							{
					...
					{//day
						...
						if (result.size() > 0)
						{
							...
							for (itt = result.begin(); itt != result.end(); ++itt)
							{
								...
								root["result"][ii]["d"] = sd[1].substr(0, 16);
								float fValue = float(atof(sd[0].c_str())) / 1000;
				...
				if (sensor == "temp") {
					...
					if (result.size() > 0)
					{
						...
						for (itt = result.begin(); itt != result.end(); ++itt)
						{
							...
							if (
								(dType == pTypeRego6XXTemp) || (dType == pTypeTEMP) || (dType == pTypeTEMP_HUM) || (dType == pTypeTEMP_HUM_BARO) || (dType == pTypeTEMP_HUM_BARO_VOLT) || (dType == pTypeTEMP_BARO) || (dType == pTypeWIND) || (dType == pTypeThermostat1) || (dType == pTypeRadiator1) ||
								((dType == pTypeRFXSensor) && (dSubType == sTypeRFXSensorTemp)) ||
								((dType == pTypeUV) && (dSubType == sTypeUV3)) ||
								((dType == pTypeGeneral) && (dSubType == sTypeSystemTemp)) ||
								((dType == pTypeThermostat) && (dSubType == sTypeThermSetpoint)) ||
					...
					if (result.size() > 0)
					{
						...
						{
							if ((dType == pTypeTEMP_HUM_BARO)||(dType == pTypeTEMP_HUM_BARO_VOLT))
							{
								if (dSubType == sTypeTHBFloat)
								{
									sprintf(szTmp, "%.1f", atof(sd[5].c_str()) / 10.0f);
					...
					{//month/year
						...
						if (result.size() > 0)
						{
							...
							for (itt = result.begin(); itt != result.end(); ++itt)
							{
								...
								fValue = float(atof(sd[1].c_str())) / 1000;
								sprintf(szTmp, "%.3f", fValue);
					...
					else if (dType == pTypeTEMP_HUM_BARO_VOLT)
					{
						...
						if(result.size())
						{
							float fValue = atof(result[0][0].c_str()) / 1000;
							sprintf(szTmp, "%.3f", fValue);

							root["result"][ii]["d"] = szDateEnd;
							root["result"][ii]["v_min"] = szTmp;
							fValue = atof(result[0][1].c_str()) / 1000;
							sprintf(szTmp, "%.3f", fValue);
							root["result"][ii]["v_max"] = szTmp;
							ii++;
						}
			...
			else if ((srange.substr(0, 1) == "2") && (srange.substr(10, 1) == "T") && (srange.substr(11, 1) == "2")) // custom range 2013-01-01T2013-12-31
			{
				...
				if (sensor == "temp") {
					...
					if ((sgraphHum == "true") &&
						(dType == pTypeTEMP_HUM) || (dType == pTypeTEMP_HUM_BARO) || (dType == pTypeTEMP_HUM_BARO_VOLT) )
					{
						sendHum = true;
					}
					if ((sgraphBaro == "true") && (
