...
void CSQLHelper::UpdateTemperatureLog()
{
	...
	std::vector<std::vector<std::string> > result;
	result=safe_query("SELECT ID,Type,SubType,nValue,sValue,LastUpdate FROM DeviceStatus WHERE (Type=%d OR Type=%d OR Type=%d OR Type=%d OR Type=%d OR Type=%d OR Type=%d OR Type=%d OR Type=%d OR Type=%d OR Type=%d OR Type=%d OR Type=%d OR Type=%d OR (Type=%d AND SubType=%d) OR (Type=%d AND SubType=%d) OR (Type=%d AND SubType=%d))",
		pTypeTEMP,
		pTypeHUM,
		pTypeTEMP_HUM,
		pTypeTEMP_HUM_BARO,
		pTypeTEMP_HUM_BARO_VOLT,
		pTypeTEMP_BARO,
		pTypeUV,
		pTypeWIND,
		pTypeThermostat1,
...
void CSQLHelper::UpdateMeter()
{
	...
	result=safe_query(
		...
		"Type=%d OR " //pTypeTEMP_HUM_BARO_VOLT
	...
	if (result.size()>0)
	{
		...
		for (itt=result.begin(); itt!=result.end(); ++itt)
		{
			...
			else if (dType==pTypeTEMP_HUM_BARO_VOLT)
			{
				...
				if (splitresults.size()<6)
					continue;
				double fValue=atof(splitresults[5].c_str())*1000.f;
				sprintf(szTmp,"%.0f",fValue);
				sValue=szTmp;
			}
	...
	for (itt=resultdevices.begin(); itt!=resultdevices.end(); ++itt)
	{
		...
		if (result.size()>0)
		{
			...
			if (
				...
				(devType!=pTypeWEIGHT)&&
				(devType!=pTypeUsage)&&
-				(devType!=pTypeTEMP_HUM_BARO_VOLT)
				)
			{
