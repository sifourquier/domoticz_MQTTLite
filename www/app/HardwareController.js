define(['app'], function (app) {
	app.controller('HardwareController', ['$scope', '$rootScope', '$location', '$http', '$interval', function ($scope, $rootScope, $location, $http, $interval) {
		...
		UpdateHardware = function (idx, Mode1, Mode2, Mode3, Mode4, Mode5, Mode6) {
		   ...
		   error: function () {
			   ShowNotify($.t('Problem updating hardware!'), 2500, true);
				...
				else if ((text.indexOf("MQTT ") >= 0) || (text.indexOf("MQTT_Lite") >= 0)) {
		...
		CreateMQTT_Lite = function(idx,name){
			$.devIdx=idx;
			
			$("#dialog-createMQTT_lite").dialog({
				autoOpen: true,
				width: 420,
				height: 250,
				modal: true,
				...
				buttons: {
					"OK": function() {
						var bValid = true;
						$( this ).dialog("close");
						var SensorName=$("#dialog-createMQTT_lite #sensorname").val();
						if (SensorName=="")
						{
							ShowNotify($.t('Please enter a Name!'), 2500, true);
							return;
						}
						var SensorType=$("#dialog-createMQTT_lite #sensortype option:selected").val();
						if (typeof SensorType == 'undefined') {
							bootbox.alert($.t('No Sensor Type Selected!'));
							return ;
						}
						var parameter=$("#dialog-createMQTT_lite #parameter").val();
						if (typeof parameter == 'undefined') {
							bootbox.alert($.t('No Sensor Type Selected!'));
							return ;
						}
						$.ajax({
							url: "json.htm?type=createMQTTsensor&idx=" + $.devIdx +
			...
			$("#dialog-createMQTT_lite").i18n();
			$("#dialog-createMQTT_lite").dialog("open");
		}
		  ...
		  success: function (data) {
			  ...
			  if (typeof data.result != 'undefined') {
				  $.each(data.result, function (i, item) {
					  ...
					  else if (HwTypeStr.indexOf("MQTT_Lite") >= 0) {
						  HwTypeStr+=' <span class="label label-info lcursor" onclick="CreateMQTT_Lite(' + item.idx + ',\'' + item.Name + '\');">' + $.t("Create Sensors") + '</span>';
					  }
			...
			$("#hardwaretable tbody").on('click', 'tr', function () {
				if ($(this).hasClass('row_selected')) {
					...
					if (anSelected.length !== 0) {
						...
						else if (data["Type"].indexOf("MQTT ") >= 0 || data["Type"].indexOf("MQTT_Lite ") >= 0) {
