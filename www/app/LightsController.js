define(['app'], function (app) {
	app.controller('LightsController', ['$scope', '$rootScope', '$location', '$http', '$interval', 'permissions', function ($scope, $rootScope, $location, $http, $interval, permissions) {
						...
						$.each(data.result, function (i, item) {
							...
							if (typeof obj != 'undefined') {
								...
								if (item.SubType == "Security Panel") {
									...
									if(item.Status=='Set Level: 0 %')
										item.Status=item.Status='Off';
									if (
										(item.Status == 'On') ||
										(item.Status == 'Chime') ||
										(item.Status == 'Group On') ||
										...
										else if (item.SubType == "_2Color") {
											img='<img src="images/' + item.Image + '48_On.png" title="' + $.t("Turn Off") + '" onclick="SetColValue(' + item.idx + ','+valDoubleSlider[item.idx+'_1']+',0,false);" class="lcursor" height="48" width="48">';
										}
										else {
										...
										if (item.SubType == "RGB") {
											...
											img='<img src="images/' + item.Image + '48_Off.png" title="' + $.t("Turn On") + '" onclick="SetColValue(' + item.idx + ','+valDoubleSlider[item.idx+'_1']+',50,false);" class="lcursor" height="48" width="48">';
										}
										else {
		...
		ShowLights = function () {
						...
						$.each(data.result, function (i, item) {
							...
							if (item.SubType == "Security Panel") {
								...
								if (item.SubType == "_2Color")
								{
									...
									valDoubleSlider[item.idx+'_0']=item.LevelInt1;
									valDoubleSlider[item.idx+'_1']=item.LevelInt2;
								}
								...
								if (
									...
									(item.Status == 'Chime') ||
									(item.Status == 'Group On') ||
									((item.Status.indexOf('Set ') == 0) && (item.Status.indexOf('Set Level: 0 %') != 0)) ||
									...
									if (item.SubType == "RGB") {
										...
										xhtm +='\t      <td id="img"><img src="images/' + item.Image + '48_On.png" title="' + $.t("Turn Off") + '" onclick="SetColValue(' + item.idx + ','+valDoubleSlider[item.idx+'_1']+',0,false);" class="lcursor" height="48" width="48"></td>';
									}
									else {
									...
									if (item.SubType == "RGB") {
										...
										xhtm +='\t      <td id="img"><img src="images/' + item.Image + '48_Off.png" title="' + $.t("Turn On") + '" onclick="SetColValue(' + item.idx + ','+valDoubleSlider[item.idx+'_1']+',50,false);" class="lcursor" height="48" width="48"></td>';
									}
									else {
							...
							if (item.SwitchType == "Dimmer") {
								if ((item.SubType.indexOf("RGBW") >= 0) || (item.SubType == "RGB")) {
									...
									xhtm+='<br><br><div id="containerslider" style="width:auto; margin-left:50px"> ';
									...
									xhtm+='<div style="float:right; margin-right:7px" class="dimsliderdouble" id="slider2" data-num="1" data-idx="' + item.idx + '" data-type="norm" data-maxlevel="' + item.MaxDimLevel + '" data-isprotected="' + item.Protected + '" data-svalue="' + item.LevelInt2 + '"></div></div>';
								}
								else {
			...
			$('#lightcontent .dimsliderdouble').slider({
				//Config
				range: "min",
				min: 1,
				max: 16,
		...
		$scope.ResizeDimSliders = function () {
			...
			$("#lightcontent .dimsliderdouble").width(width/2-6);
		}
